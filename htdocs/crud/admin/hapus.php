...
if(isset($_GET['id'])){
    ...
    $id = $_GET['id'];
    $cek = mysqli_query($con, "SELECT * FROM tabel_biodata WHERE id='$id'");

    if(mysqli_num_rows($cek)==0){
        echo '<script>window.history.back()</script>';
    }else{
        $del = mysqli_query($con, "DELETE FROM tabel_biodata WHERE id='$id'");
        if($del){
            echo 'Data siswa berhasil dihapus';
            echo '<a href="data.php">Kembali</script>';
        }else{
